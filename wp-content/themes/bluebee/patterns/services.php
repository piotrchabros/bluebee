...
<section class="wp-block-group alignfull bb-services bb-section has-background" id="services" style="background-color:var(--wp--preset--color--cream);padding-top:var(--wp--preset--spacing--80);padding-right:var(--wp--preset--spacing--60);padding-bottom:var(--wp--preset--spacing--80);padding-left:var(--wp--preset--spacing--60)">
	...
	<div class="wp-block-query bb-services-query">
		...
		<!-- wp:query-no-results -->
		<!-- wp:group {"className":"bb-services__grid bb-services__grid--fallback","layout":{"type":"default"}} -->
		<div class="wp-block-group bb-services__grid bb-services__grid--fallback">
			...
			foreach ( $fallback_services as $i => $svc ) :
			?>
			<div class="bb-service-card bb-animate bb-animate--delay-<?php echo $i + 1; ?>" style="padding:2.5rem 2rem;border-left:1px solid var(--wp--preset--color--border);position:relative">
				...
				<p style="margin-top:2rem"><a href="#services" style="font-family:var(--wp--preset--font-family--heading);font-size:0.75rem;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:var(--wp--preset--color--blue);text-decoration:none"><?php echo esc_html__( 'Learn More →', 'bluebee' ); ?></a></p>
			</div>
			<?php endforeach; ?>
			<!-- /wp:html -->
		</div>
